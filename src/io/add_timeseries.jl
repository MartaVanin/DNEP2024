...
function add_timeseries_33bus!(data::Dict; n_timesteps=527040)::Dict
    ...
    for t in 1:n_timesteps
        ...
        for (l, load) in mn_data["nw"]["$t"]["load"]
            ...
            if haskey(load, "was_gen") # there is only one generation profile for all
                load["pd"] = fill(-ts_gen[t, 1]*264.75/length(conns), length(conns)) ./ data["settings"]["sbase_default"] # generators all have size 264.75
                ...
                if original_load_id == 1
                    ...
                    load["pd"] = fill(ts_dem[t, original_load_id]/length(conns), length(conns)) ./ data["settings"]["sbase_default"] 
                    load["qd"] = fill(ts_dem[t, original_load_id]/length(conns), length(conns)) ./ data["settings"]["sbase_default"] * tan(acos(pf[original_load_id, 1])) 
                end
            end
        end
    end
...
function add_timeseries_irish!(data::Dict)
    ts_gen = CSV.read(joinpath(BASE_DIR, "data/IrishMVGrid/Profiles/GenerationProfile.csv"), _DF.DataFrame, header = 0) # unitless multipliers of rated generator size
    ts_dem = CSV.read(joinpath(BASE_DIR, "data/IrishMVGrid/Profiles/DemandProfiles.csv"), _DF.DataFrame, header = 0)    # unitless multipliers of rated load size
    ...
    for t in 1:size(ts_gen)[1]
        mn_data["nw"]["$t"] = Dict{String, Any}()
        mn_data["nw"]["$t"] = deepcopy(data)
        for (_, load) in mn_data["nw"]["$t"]["load"]
            if haskey(load, "was_gen")
                load["pd"] = fill(-ts_gen[t, 1]/length(conns), length(conns)) ./ data["settings"]["sbase_default"]
                load["qd"] = fill(0., length(conns)) # generation always have PF = 1
            else
                original_load_id = parse(Int, load["name"]) # as in openDSS. PMD scrambles the ids
                load["pd"] = fill(ts_dem[t, original_load_id]/length(conns), length(conns)) ./ data["settings"]["sbase_default"] * load_info.kW[original_load_id]
                load["qd"] = fill(ts_dem[t, original_load_id]/length(conns), length(conns)) ./ data["settings"]["sbase_default"] * load_info.kvar[original_load_id]
            end
        end
    end
    return mn_data
